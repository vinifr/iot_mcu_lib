...
/*??pthread_mutex_t*/uint32_t global_alloc_free_lock = 1/*??PTHREAD_MUTEX_INITIALIZER*/;
...
libwebsock_wait(libwebsock_context *ctx)
{
  ...
  //?sig_event = evsignal_new(ctx->base, /*??SIGINT*/1, libwebsock_handle_signal, (void *)ctx);
  //?event_add(sig_event, NULL);
  //?sig_event = evsignal_new(ctx->base, /*??SIGUSR2*/0, libwebsock_handle_signal, (void *)ctx);
  //?event_add(sig_event, NULL);
  ctx->running = 1;
...
libwebsock_bind(libwebsock_context *ctx, char *listen_host, char *port)
{
  ...
  /*??evutil_socket_t*/uint32_t sockfd;
  int yes = 1;
...
void 
libwebsock_bind_socket(libwebsock_context *ctx, /*??evutil_socket_t*/uint32_t sockfd)
{
  //??struct event *listener_event = event_new(ctx->base, sockfd, /*??EV_READ*/0x02 | /*??EV_PERSIST*/0x10, libwebsock_handle_accept, (void *) ctx);
