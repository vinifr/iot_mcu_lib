...
void libwebsock_handle_signal(/*??evutil_socket_t*/uint32_t sig, short event, void *ptr) {
...
void
libwebsock_insert_into_thread_list(libwebsock_client_state *state, /*??pthread_t*/uint32_t *thread, enum WS_THREAD_TYPE type)
{
...
void libwebsock_cleanup_thread_list(/*??evutil_socket_t*/uint32_t sock, short what, void *arg) {
	thread_state_wrapper *wrapper = arg;
	thread_info *tinfo, *current = NULL;
	/*??pthread_t*/uint32_t current_thread;
	...
	for (tinfo = state->tlist; tinfo != NULL; tinfo = tinfo->next) {
		current = tinfo;
		current_thread = *((/*pthread_t*/uint32_t *)tinfo->thread);
...
void libwebsock_shutdown_after_send_cb(/*??evutil_socket_t*/uint32_t fd, short what,
		void *arg) {
	libwebsock_client_state *state = (libwebsock_client_state *) arg;
	libwebsock_shutdown(state);
}

void libwebsock_post_shutdown_cleanup(/*??evutil_socket_t*/uint32_t fd, short what, void *arg) {
	libwebsock_client_state *state = (libwebsock_client_state *) arg;
	/*??pthread_t*/uint32_t this_thread;
	if (!state) {
		return;
	}
	...
	while (info != NULL) {
		this_thread = *((/*pthread_t*/uint32_t *)info->thread);
#ifdef LIBWEBSOCK_DEBUG
...
void libwebsock_handle_accept(/*evutil_socket_t*/uint32_t listener, short event, void *arg) {
	libwebsock_context *ctx = arg;
	libwebsock_client_state *client_state;
	struct bufferevent *bev;
	struct /*??sockaddr_storage*/sockaddr ss;
	...
	client_state->sa = (struct /*??sockaddr_storage*/sockaddr *) lws_malloc(
			sizeof(struct /*??sockaddr_storage*/sockaddr));
	client_state->ctx = (void *) ctx;
	memcpy(client_state->sa, &ss, sizeof(struct /*??sockaddr_storage*/sockaddr));
	//??evutil_make_socket_nonblocking(fd);
	//??bev = bufferevent_socket_new(ctx->base, fd,/*??BEV_OPT_CLOSE_ON_FREE*/(1<<0) | /*??BEV_OPT_THREADSAFE*/1<<1);
...
void libwebsock_dispatch_message(libwebsock_client_state *state) {
	...
	*(message_payload) = '\0';
	...
	wrapper->state = state;
	wrapper->msg = msg;
	/*??pthread_t*/uint32_t *tptr = lws_malloc(sizeof(/*??pthread_t*/uint32_t));
...
//		/*??pthread_t uint32_t *onopen_thread = (/*??pthread_t uint32_t *) lws_malloc(sizeof(/*pthread_t uint32_t));
...
void libwebsock_handshake(struct bufferevent *bev, void *ptr) {
	...
	if (!str) {
		//-ok-state->data = (libwebsock_string *) lws_calloc(str->data_sz);
		//-original-state->data = (libwebsock_string *) lws_calloc(sizeof(libwebsock_string));
		str = state->data;
